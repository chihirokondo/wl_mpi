...
MPIV::MPIV(int numprocs, int myid, int multiple)
    : numprocs_(numprocs),
      myid_(myid),
      multiple_(multiple),
      num_windows_(numprocs/multiple),
      num_windows_mod2_((numprocs/multiple)%2) {}
...
void MPIV::create_local_communicator() {
  ...
  std::vector<MPI_Comm> local_comm_buf(num_windows_-1);
...
void MPIV::set_local_id() {
  local_id_ = {-1, -1};
  int comm_id_tmp;
  if (!((num_windows_mod2_==1)&&(myid_>=numprocs_-multiple_))) {
